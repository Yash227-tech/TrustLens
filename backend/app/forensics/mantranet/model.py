"""ManTraNet PyTorch port — vendored from RonyAbecidan/ManTraNet-pytorch.

Stripped of unused imports (matplotlib, pytorch_lightning, cv2.detail_ImageFeatures)
and of the demo check_forgery() function which depended on matplotlib.
Model architecture and pretrained weights are unchanged.
"""
...
class IMTFE(nn.Module):
    ...
    def __init__(self, in_channel=3,device=device):
        ...
        # Patched: original code used relative path 'IMTFEv4.pt'. Resolve to vendored weights/ directory.
        from pathlib import Path as _P
        _imtfe_path = _P(__file__).parent / "weights" / "IMTFEv4.pt"
        self.SRMConv2D.weight.data = torch.load(str(_imtfe_path), map_location=device, weights_only=False)['SRMConv2D.weight']
...
# NOTE: original repo had a check_forgery() demo function here using matplotlib.
# Removed to avoid the matplotlib dependency — our wrapper module handles inference.
